  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div><p className="text-sm font-bold text-[var(--ink)]">{isLoading ? "Loading catalog…" : `${projects.length} loaded projects`}</p><p className="mt-0.5 text-[10px] text-[var(--muted)]">One canonical record per project · exact provenance retained</p></div>
        <div className="flex items-center gap-2"><label><span className="sr-only">Sort projects</span><select className={selectClass} value={sort} onChange={(event) => updateParams({ sort: event.target.value })}><option value="recently_seen">Newest sightings</option><option value="repository_activity">Repository activity</option><option value="newest">Recently added</option><option value="name">Name A–Z</option></select></label><div className="flex rounded-lg border border-[var(--line)] bg-[var(--surface)] p-0.5 shadow-sm" aria-label="Inventory view"><button className={cn("grid size-8 place-items-center rounded-md text-[var(--muted)]", view === "cards" && "bg-[var(--surface-raised)] text-[var(--ink)] shadow-sm")} onClick={() => updateParams({ view: "cards" })} aria-label="Card view" aria-pressed={view === "cards"}><Grid2X2 className="size-3.5" /></button><button className={cn("grid size-8 place-items-center rounded-md text-[var(--muted)]", view === "list" && "bg-[var(--surface-raised)] text-[var(--ink)] shadow-sm")} onClick={() => updateParams({ view: "list" })} aria-label="Compact list view" aria-pressed={view === "list"}><List className="size-3.5" /></button></div></div>
      </div>

      {isLoading ? (
        <Card className="grid min-h-72 place-items-center p-8"><div className="text-center"><LoaderCircle className="mx-auto size-6 animate-spin text-[var(--accent)] motion-reduce:animate-none" /><p className="mt-3 text-xs text-[var(--muted)]">Reading the shared catalog</p></div></Card>
      ) : error && projects.length === 0 ? (
        <Card className="grid min-h-72 place-items-center p-8 text-center" role="alert"><div><ArchiveX className="mx-auto size-6 text-[var(--danger)]" /><h2 className="mt-3 text-base font-bold">Catalog unavailable</h2><p className="mt-1 max-w-md text-xs leading-5 text-[var(--muted)]">{error}</p><Button className="mt-4" variant="secondary" size="sm" onClick={() => setReloadToken((value) => value + 1)}>Try again</Button></div></Card>
      ) : projects.length === 0 ? (
        <Card className="grid min-h-80 place-items-center p-8 text-center"><div><span className="mx-auto grid size-12 place-items-center rounded-xl bg-[var(--surface-raised)] text-[var(--muted)]"><ArchiveX className="size-5" /></span><h2 className="mt-4 text-base font-bold text-[var(--ink)]">No projects match</h2><p className="mt-1 max-w-sm text-xs leading-5 text-[var(--muted)]">Try a broader term or remove one of the active filters. Nothing in the catalog was changed.</p><Button className="mt-4" variant="secondary" size="sm" onClick={() => router.replace(pathname)}><Check className="size-3.5" /> Reset filters</Button></div></Card>
      ) : (
        <>
          ...
          {view === "cards" ? <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-3">{projects.map((project) => <ProjectCard key={project.id} project={project} />)}</div> : <div className="space-y-2">{projects.map((project) => <ProjectCard key={project.id} project={project} compact />)}</div>}
